...
import { TextLinkAtoms } from "./text-link.config";

export type BaseTextLinkProps = PressableProps & {
  children?: string;
};

const CustomTextLink = React.memo(
  React.forwardRef(
    (
      {
        children,
        atoms,
        ...rest
      }: MoleculeComponentProps<"TextLink", BaseTextLinkProps, TextLinkAtoms>,
      ref: any
    ) => {
