...
2. Provides an easier way to pass styles via props (`backgroundColor` can be passed as a prop instead of using [React Stylesheet](https://reactjs.org/docs/faq-styling.html) or inline styles). Shorthand props are also supported (`bgColor` instead of `backgroundColor`).
