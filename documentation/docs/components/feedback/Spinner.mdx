...
The `size` prop allows you to adjust the size of the spinner. You can set the value to the keys available in <t>PearlTheme.components.Spinner["sizes"]</t>, which are <t>"s"</t>, <t>"m"</t>, <t>"l"</t>, and <t>"xl"</t> in the [default theme](#default-component-config).
...
In the example above, as soon as the screen loads, the [Text](../typography/Text) component would be invisible for 2 seconds while the `Spinner` is animating. After 2 seconds, the `Spinner` gets removed from the DOM and the [Text](../typography/Text) component is rendered.

### Overriding Styles

The `Spinner` component supports a [variety of style props](#supported-style-props) which can be used to override the pre-defined variant style in the theme. Manual style props passed into the component have a higher precedence than the [default component config](#default-component-config).
